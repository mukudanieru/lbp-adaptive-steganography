...
def extract_3msb(green_channel: np.ndarray) -> np.ndarray:
    """
    Convert a green channel (uint8, 0-255) to its 3-MSB representation (0-7).

    Args:
        green_channel: NumPy array of shape (H, W), dtype uint8
                       representing the green channel of an RGB image.

    Returns:
        NumPy array of shape (H, W), dtype uint8, values in range 0-7
    """
    if green_channel.dtype != np.uint8:
        raise ValueError("green channel must be of dtype uint8")

    # Keep only the 3 most significant bits of the green intensity
    return np.right_shift(green_channel, 5)
